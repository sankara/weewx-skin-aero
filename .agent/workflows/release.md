...
2. **Get Context**:
    ...
    * **README & Packaging**: Note that `uv run aero-version` automatically updates the versioned download link in `README.md`. Also, `uv run aero-package` generates both a versioned zip (e.g., `weewx-aero-v1.4.0.zip`) and a generic `weewx-aero.zip` to support stable "latest" links.
    * Git Log & Last Tag:
      ```bash
